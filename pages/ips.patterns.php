...
$form .= '<i class="fa fa-question-circle text-info" title="URL-Pfad oder RegEx-Pattern für Bedrohungserkennung. Beispiele: /evil.php, ../../../, /union\s+select/i" data-toggle="tooltip"></i>';
$form .= '</label>';
...
$form .= '<i class="fa fa-question-circle text-info" title="Optionale Beschreibung zur Dokumentation des Patterns (z.B. \'SQL Injection Schutz\')" data-toggle="tooltip"></i>';
$form .= '</label>';
...
$form .= '<i class="fa fa-question-circle text-info" title="LOW: Nur Protokollierung | MEDIUM: 15min Sperrung | HIGH: 1h Sperrung | CRITICAL: Permanente Sperrung" data-toggle="tooltip"></i>';
$form .= '</label>';
$form .= '<select class="form-control" id="severity" name="severity">';
$currentSeverity = $editPattern['severity'] ?? 'medium';
$form .= '<option value="low"' . ($currentSeverity === 'low' ? ' selected' : '') . ' title="Nur Protokollierung, keine Sperrung">' . $addon->i18n('upkeep_ips_severity_low') . ' (Log only)</option>';
$form .= '<option value="medium"' . ($currentSeverity === 'medium' ? ' selected' : '') . ' title="15 Minuten temporäre Sperrung">' . $addon->i18n('upkeep_ips_severity_medium') . ' (15min)</option>';
$form .= '<option value="high"' . ($currentSeverity === 'high' ? ' selected' : '') . ' title="1 Stunde temporäre Sperrung">' . $addon->i18n('upkeep_ips_severity_high') . ' (1h)</option>';
$form .= '<option value="critical"' . ($currentSeverity === 'critical' ? ' selected' : '') . ' title="Permanente Sperrung">' . $addon->i18n('upkeep_ips_severity_critical') . ' (Permanent)</option>';
$form .= '</select>';
